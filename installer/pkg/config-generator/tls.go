...
func (c *ConfigGenerator) GenerateTLSConfig(clusterDir string) error {
	...
	if c.CA.RootCAKeyPath == "" && c.CA.RootCACertPath == "" {
		caCert, caKey, err = generateRootCerts(clusterDir)
		if err != nil {
	...
	// generate kube CA
	if err := generateCACerts(clusterDir, caKey, caCert, kubeCAKeyPath, kubeCACertPath, "kube-ca", "bootkube", x509.KeyUsageKeyEncipherment|x509.KeyUsageDigitalSignature|x509.KeyUsageCertSign); err != nil {
		return fmt.Errorf("failed to generate kube CAs: %v", err)
	}
	// generate aggregator CA
	if err := generateCACerts(clusterDir, caKey, caCert, aggregatorCAKeyPath, aggregatorCACertPath, "aggregator", "bootkube", x509.KeyUsageKeyEncipherment|x509.KeyUsageDigitalSignature|x509.KeyUsageCertSign); err != nil {
		return fmt.Errorf("failed to generate aggregator CAs: %v", err)
	}
	// generate service-serving CA
	if err := generateCACerts(clusterDir, caKey, caCert, serviceServiceCAKeyPath, serviceServiceCACertPath, "service-serving", "bootkube", x509.KeyUsageKeyEncipherment|x509.KeyUsageDigitalSignature|x509.KeyUsageCertSign); err != nil {
		return fmt.Errorf("failed to generate kube CAs: %v", err)
	}
	return nil
}
...
func generateRootCerts(clusterDir string) (cert *x509.Certificate, key *rsa.PrivateKey, err error) {
	...
	caCert, err := rootCA(clusterDir, caKey)
	if err != nil {
...
func rootCA(path string, key *rsa.PrivateKey) (*x509.Certificate, error) {
